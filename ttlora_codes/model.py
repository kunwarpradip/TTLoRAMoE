...
def train_with_ray():
    ...
    train_dataset, val_dataset = get_tokenizer("./checkpoints/llama", dataset)
    ...
    model = AutoModelForQuestionAnswering.from_pretrained("./checkpoints/llama")
    ...
    assign_loratt = partial(LoRATTLinearWrapper, tt_rank=tt_rank, alpha=loratt_alpha)

    #Assigning loratt to the transformer layers (self attention - for Q and V weights)
    for layer in model.transformer.layers:
        if loratt_query:
            layer.self_attn.q_proj = assign_loratt(layer.self_attn.q_proj, 0, tt_shape)
        if loretta_value:
            layer.self_attn.v_proj = assign_loratt(layer.self_attn.v_proj,2, tt_shape)
    ...
    #loading model (Llama70B), model path used to load tokenizer, learning rate
    lightning_model = CustomLightningModule(model,"./checkpoints/llama",5e-5)
