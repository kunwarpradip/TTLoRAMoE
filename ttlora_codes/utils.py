...
def load_dataset_():
    ...
    # Select a subset of the dataset (e.g., the first 1000 examples from train and validation)
    train_subset = dataset['train'].select(range(1000))
    val_subset = dataset['validation'].select(range(1000))

    # Combine the subsets into a single dictionary
    subset = {
        'train': train_subset,
